import XCTest
@testable import ContactLensLog

@MainActor
final class ContactLensLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        let added = store.add(LensPairEntry(title: "Test entry"))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        while store.entries.count < Store.freeEntryLimit {
            _ = store.add(LensPairEntry(title: "Filler"))
        }
        let added = store.add(LensPairEntry(title: "Overflow"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        let a = LensPairEntry(title: "A")
        let b = LensPairEntry(title: "B")
        store.entries = [a, b]
